...
const GITHUB_RAW_URL = "https://raw.githubusercontent.com/Luxxy-appocalype/FxZTeams/refs/heads/main/nvrsx.js"; 
...
(async () => {
    ...
    const checkUpdate = async () => {
        try {
            console.log(chalk.yellow("🔍 Checking for updates from GitHub..."));
            const res = await axios.get(GITHUB_RAW_URL);
            const newCode = res.data;
            const currentCode = fs.readFileSync(__filename, 'utf8');
            if (newCode.length > 100 && newCode !== currentCode) {
                console.log(chalk.green("🔄 New update found! Overwriting file..."));
                fs.writeFileSync(__filename, newCode, 'utf8');
                console.log(chalk.bold.green("✅ Update applied! Restarting..."));
                process.exit(0); 
            } else {
                console.log(chalk.blue("✅ Script is up to date."));
            }
        } catch (err) {
