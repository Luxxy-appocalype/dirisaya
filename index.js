...
// ================= GLOBAL FUNCTIONS (PUSH/DEL/SECRET) =================
global.manageSecret = async (action, newKey, master) => {
    ...
    if (action === 'check') {
        return { success: true, msg: `🔑 Secret Key saat ini: *${getStoredKey() || "KOSONG"}*` };
    }
};
...
const runUpdateLogic = async () => {
    ...
    console.log("1. Update (Cek pembaruan GitHub)");
    console.log("2. Bypass (Langsung jalankan bot)");
    
    let choice = await askQuestion(chalk.cyan("Pilih (1/2) atau tunggu 5 detik untuk auto: "));
    if (!choice) {
        console.log(chalk.magenta("\n🕒 Waktu habis, mengambil opsi dari Panel..."));
    ...
    if (choice === '1') {
        ...
        try {
            console.log(chalk.cyan("🚀 Mengecek GitHub..."));
            let isUpdated = false;
            ...
            console.log(chalk.red("❌ Error Update: " + e.message));
        }
    } else {
        console.log(chalk.yellow("⚠️ Menjalankan mode Bypass (Tanpa Update)."));
    }
};
