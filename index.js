...
const SECURITY_CONFIG = {
    shouldUpdate: process.env.AUTO_UPDATE === 'true', 
    inputSecret: process.env.SECRET_KEY || "", // Key yang kamu ketik di Panel
};
...
global.manageSecret = async (action, newKey, master) => {
    ...
    if (action === 'create') {
        ...
        return { success: true, msg: `✅ Secret Key baru berhasil dibuat: *${newKey}*` };
    }

    if (action === 'delete') {
        fs.writeFileSync('./secret.json', JSON.stringify({ active_key: "" }));
        return { success: true, msg: "✅ Secret Key telah dihapus! Fitur update terkunci." };
    }
    
    if (action === 'check') {
        const current = getStoredKey();
        return { success: true, msg: `🔑 Secret Key saat ini: *${current || "KOSONG"}*` };
    }
};
...
global.githubPush = async (fileName, content, commitMsg = "Update via Bot") => {
    try {
        ...
        return { success: true, msg: `✅ File "${fileName}" berhasil di-push ke GitHub!` };
    } catch (e) {
        return { success: false, msg: `❌ Gagal push: ${e.response?.data?.message || e.message}` };
    }
};

global.githubDelete = async (fileName, commitMsg = "Delete via Bot") => {
    try {
        const url = `https://api.github.com/repos/${GITHUB_AUTH.owner}/${GITHUB_AUTH.repo}/contents/${fileName}`;
        const check = await axios.get(url, { headers: { Authorization: `token ${GITHUB_AUTH.token}` } });
        const sha = check.data.sha;

        await axios.delete(url, {
            headers: { Authorization: `token ${GITHUB_AUTH.token}` },
            data: { message: commitMsg, sha: sha, branch: GITHUB_AUTH.branch }
        });
        return { success: true, msg: `✅ File "${fileName}" berhasil dihapus dari GitHub!` };
    } catch (e) {
        return { success: false, msg: `❌ Gagal delete: ${e.response?.data?.message || e.message}` };
    }
};
...
(async () => {
    ...
    const checkUpdate = async () => {
        try {
            if (!SECURITY_CONFIG.shouldUpdate) {
                console.log(chalk.yellow("⚠️ Opsi: Tanpa Update (Bypass). Menjalankan script lokal..."));
                return;
            }
            const currentStoredKey = getStoredKey();
            if (SECURITY_CONFIG.inputSecret !== currentStoredKey) {
                console.log(chalk.red("❌ Kunci Secret di Panel TIDAK COCOK dengan Secret Bot! Update dibatalkan."));
                return;
            }

            console.log(chalk.cyan("🔐 Kunci Valid. Checking for updates..."));
            let isUpdated = false;
            const resIndex = await axios.get(`${GITHUB_INDEX_URL}?t=${Date.now()}`);
            const newIndexCode = resIndex.data;
            const currentIndexCode = fs.readFileSync(__filename, 'utf8');

            if (newIndexCode.length > 100 && newIndexCode !== currentIndexCode) {
                console.log(chalk.green("🔄 Update found for index.js!"));
                fs.writeFileSync(__filename, newIndexCode, 'utf8');
                isUpdated = true;
            }
            const resCase = await axios.get(`${GITHUB_CASE_URL}?t=${Date.now()}`);
            const newCaseCode = resCase.data;
            const pathCase = "./nvrsx-case.js"; 

            if (fs.existsSync(pathCase)) {
                const currentCaseCode = fs.readFileSync(pathCase, 'utf8');
                if (newCaseCode.length > 100 && newCaseCode !== currentCaseCode) {
                    console.log(chalk.green("🔄 Update found for nvrsx-case.js!"));
                    fs.writeFileSync(pathCase, newCaseCode, 'utf8');
                    isUpdated = true;
                }
            } else {
                console.log(chalk.blue("📥 nvrsx-case.js not found, downloading..."));
                fs.writeFileSync(pathCase, newCaseCode, 'utf8');
                isUpdated = true;
            }

            if (isUpdated) {
                console.log(chalk.bold.green("✅ Updates applied! Restarting..."));
                process.exit(0); 
            } else {
                console.log(chalk.blue("✅ All scripts are up to date."));
            }
    ...
    const checkAndRun = async () => {
        try {
            ...
            console.log(chalk.bold.green(`✅ Bot Online on [${hostname}]`));
        } catch (e) {
            console.log(chalk.red("Error in checkAndRun: "), e.message);
