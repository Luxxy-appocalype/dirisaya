...
// ================= SISTEM SECRET (Cek File Lokal) =================
const getStoredKey = () => {
    try {
        if (!fs.existsSync('./secret.json')) return "KOSONG";
        return JSON.parse(fs.readFileSync('./secret.json')).active_key;
    } catch (e) { return "ERROR"; }
};

// ================= LOGIKA PEMBARUAN OTOMATIS (Tanpa Readline) =================
const runUpdateLogic = async () => {
    return new Promise((resolve) => {
        console.log(chalk.bold.yellow("\n--- OPSI PEMBARUAN ---"));
        console.log("1. Update (Cek GitHub)");
        console.log("2. Bypass (Tanpa Update)");
        console.log(chalk.cyan("Pilih (1/2) atau tunggu 20 detik..."));
        const timeout = setTimeout(() => {
            process.stdin.pause();
            console.log(chalk.magenta("\n🕒 Timeout, menggunakan setting Panel..."));
            handleChoice(process.env.AUTO_UPDATE === 'true' ? '1' : '2');
        }, 20000);
        const handleChoice = async (choice) => {
            if (choice.trim() === '1') {
                ...
                if (inputSecret !== storedKey) {
                    console.log(chalk.red("❌ Secret Panel SALAH. Update dibatalkan."));
                    resolve();
                    return;
                }
                try {
                    console.log(chalk.cyan("🚀 Mendownload pembaruan..."));
                    console.log(chalk.green("✅ Sistem siap."));
                } catch (e) {
        ...
        process.stdin.once('data', (data) => {
            ...
            process.stdin.pause();
            handleChoice(data.toString().trim());
        });
    });
};
