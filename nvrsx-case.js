...
//================ FITUR KHUSUS OWNER ================//
   ...
   bot.command('deploy', async (ctx) => {
    ...
    bot.command("pushfile", async (ctx) => {
        ...
        if (!allowed) return;
        const args = ctx.message.text.split(' ');
        if (args.length < 2) return ctx.reply("❌ Masukkan nama filenya! Contoh: /pushfile index.js");
        const namaFile = args[1];
        if (!fs.existsSync(`./${namaFile}`)) return ctx.reply("❌ File tidak ditemukan di server.");
