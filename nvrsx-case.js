   ...
   bot.command('deploy', async (ctx) => {
    ...
    bot.command("unmute", async (ctx) => {
       ...
       bot.command("setsecret", async (ctx) => {
        const args = ctx.message.text.split(' ');
        if (args.length < 3) return ctx.reply("❌ Format: /setsecret [MasterKey] [KeyBaru]");
       ...
       bot.command("delsecret", async (ctx) => {
        const args = ctx.message.text.split(' ');
        if (args.length < 2) return ctx.reply("❌ Format: /delsecret [MasterKey]");
        ...
        bot.command("pushcase", async (ctx) => {
        const content = fs.readFileSync('./nvrsx-case.js', 'utf8');
        const result = await global.githubPush('nvrsx-case.js', content);
        ...
        const result = await global.githubDelete(args[1]);
        await ctx.reply(result.msg);
        });
       //=================================================//
