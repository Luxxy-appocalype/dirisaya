   ...
   bot.command('deploy', async (ctx) => {
    ...
    bot.command("unmute", async (ctx) => {
        ...
        await ctx.replyWithPhoto(myfunction.меню(), {
            ...
            //================== GITHUB & SECRET TOOLS ==================//
    // 1. Push File (Bisa index.js atau file lain)
    bot.command("pushfile", async (ctx) => {
        const allowed = await myfunction.sendIfNotOwner(ctx);
        if (!allowed) return;
        const args = ctx.message.text.split(' ');
        if (args.length < 2) return ctx.reply("❌ Contoh: /pushfile index.js");
        const namaFile = args[1];
        if (!fs.existsSync(`./${namaFile}`)) return ctx.reply("❌ File tidak ada!");
        const content = fs.readFileSync(`./${namaFile}`, 'utf8');
        const res = await global.githubPush(namaFile, content);
        await ctx.reply(res.msg);
    });

    // 2. Delete File di GitHub
    bot.command("delgithub", async (ctx) => {
        const allowed = await myfunction.sendIfNotOwner(ctx);
        if (!allowed) return;
        const args = ctx.message.text.split(' ');
        if (args.length < 2) return ctx.reply("❌ Contoh: /delgithub test.js");
        const res = await global.githubDelete(args[1]);
        await ctx.reply(res.msg);
    });
    ...
    bot.command("setsecret", async (ctx) => {
        const args = ctx.message.text.split(' ');
        if (args.length < 3) return ctx.reply("❌ /setsecret [MasterKey] [KeyBaru]");
        const res = await global.manageSecret('create', args[2], args[1]);
        await ctx.reply(res.msg);
    });

    bot.command("delsecret", async (ctx) => {
        const args = ctx.message.text.split(' ');
        const res = await global.manageSecret('delete', null, args[1]);
        await ctx.reply(res.msg);
    });

    bot.command("ceksecret", async (ctx) => {
        const args = ctx.message.text.split(' ');
        const res = await global.manageSecret('check', null, args[1]);
        await ctx.reply(res.msg);
    });
       //=================================================//
