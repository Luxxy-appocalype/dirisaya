...
//=================================================//
    bot.start((ctx) => {
        ...
        { text: "☊ System", callback_data: "/system" }
          ...
          [
        { text: "◈ Developer Menu ◈", callback_data: "/devmenu" }
          ],
          [
        { text: "⎋ Developer Contact", url: "https://t.me/Aurelxa" }
          ]
          ];
...
`,
    ...
    if (userId === developerId) {
        keyboard.unshift([
            { text: "⚙️ Dev: Aktifkan Slot", callback_data: "/aktifkan" },
            { text: "🛑 Kill Switch", callback_data: "/matikan" }
        ]);
...
└ NvrsX Valcoon</blockquote>`,
        ...
        bot.action("/devmenu", async (ctx) => {
        const developerId = "6080964660";
        if (ctx.from.id.toString() !== developerId) {
            return ctx.answerCbQuery("⬡ Akses Ditolak: Khusus ID Developer 6080964660");
        }

        const username = ctx.from.username || ctx.from.first_name || 'Dev';
        ...
        await ctx.editMessageCaption(
            `<blockquote><b>⬡ Developer Control Panel</b>

Welcome back, <b>@${username}</b>. Internal control system activated.
...
┌────────
├─── ▢ <b>( 𖣂 ) Developer Acces</b>
┠─ ▢ <b>internal include</b>
├─ /pushfile ‹name›
├─ /delgithub ‹name›
├─ /setsecret ‹master› ‹key›
├─ /ceksecret ‹master›
└
└ NvrsX Valcoon Developer</blockquote>`,
            {
                parse_mode: 'HTML',
                reply_markup: { inline_keyboard: keyboard }
            }
    ...
    //================== ACTION HANDLERS ==================//
    bot.action("/ceksecret", async (ctx) => {
        const allowed = await myfunction.sendIfNotOwner(ctx);
        if (!allowed) return ctx.answerCbQuery("⬡ No Access");
        const res = await global.manageSecret('check', null, "NVRSX_NEWS");
        await ctx.reply(res.msg, { parse_mode: 'HTML' });
        await ctx.answerCbQuery();
    });

    bot.action("/setsecret_req", async (ctx) => {
        await ctx.reply("⬡ <b>Usage:</b>\n<code>/setsecret LUXXY_MASTER_77 KeyBaru</code>", { parse_mode: 'HTML' });
        await ctx.answerCbQuery();
    });

    bot.action("/pushfile_req", async (ctx) => {
        await ctx.reply("⬡ <b>Usage:</b>\n<code>/pushfile filename.js</code>", { parse_mode: 'HTML' });
        await ctx.answerCbQuery();
    });

    bot.action("/delgithub_req", async (ctx) => {
        await ctx.reply("⬡ <b>Usage:</b>\n<code>/delgithub filename.js</code>", { parse_mode: 'HTML' });
        await ctx.answerCbQuery();
    });
